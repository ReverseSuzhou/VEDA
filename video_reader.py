...
class Split():
    ...
    def add_vid(self, paths, gt_a):   # 以path的方式保存视频保存在self.video_loader.dataset.train_split.videos中，对应的类别是gt_a
        self.videos.append(paths)  
        self.gt_a_list.append(gt_a)   # 以list的方式保存的是gt_a也就是类别

    def get_rand_vid(self, label, idx=-1):  # 从label类别中选择一个视频，返回这个视频和它的index
        match_idxs = []
    ...
    def get_num_videos_for_class(self, label):   # 返回label类别视频的数目
...
class VideoDataset(torch.utils.data.Dataset):
    ...
    """Setup crop sizes/flips for augmentation during training and centre crop for testing""" #设置裁剪尺寸/翻转以在训练期间进行增强，并设置中心裁剪以进行测试
